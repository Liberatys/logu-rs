...
impl Evaluator {
    ...
    pub fn add_input(&mut self, input: input::Entry) {
        let input_name: String = input.name.clone();
        self.inputs.push(input);
        self.headers.push(input_name);
    }

    pub fn add_output(&mut self, output: input::Entry) {
        let output_name: String = output.name.clone();
        self.outputs.push(output);
        self.headers.push(output_name);
    }
    ...
    pub fn evaluate_matrix(&self) -> Vec<Vec<i32>> {
        ...
        for x in 0..vec_vector_length {
            for t in 0..self.expressions.len() {
                let result = self.expressions[t as usize]
                    .evaluate_expression(&evaluation_matrix[x as usize]);
                match result {
                    expression::ExpressionResult::Ok(v) => {
                        for (key, value) in v {
                            evaluation_matrix[x as usize]
                                [(key + input_index_range as i32) as usize] = value
                        }
                    }
